...
if(isset($_POST['update']))
{
    ...
    $location = "./upload/$imgUpdate_name";
    move_uploaded_file($img_temp, $location);

    if (empty($_FILES['image']['name'])) {
    $updateAdminData = "UPDATE `admin` SET `name`='$name',`age`=$age,`address`='$address',`phone`='$phone',`email`='$email',`password`='$password',`image`='$row[7]',`role_id`='1' WHERE `id` = $id ";
    }
else {
    ...
    move_uploaded_file($img_temp, $location);
    $updateAdminData = "UPDATE `admin` SET `name`='$name',`age`=$age,`address`='$address',`phone`='$phone',`email`='$email',`password`='$password',`image`='/project/screens/profile/upload/$img_name',`role_id`='1' WHERE `id` = $id ";
};
    $doneUpdateAdminData = mysqli_query($connection, $updateAdminData);
    go("/screens/profile/my_profile.php");
}

?>

<form method="post" enctype="multipart/form-data">
<div class="form-group"><img width="200" src="<?= $row[7] ?>" class="img-thumbnail" > <input type="file" name="image"></div>
